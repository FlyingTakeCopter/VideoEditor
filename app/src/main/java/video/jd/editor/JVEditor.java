...
import video.jd.Jni.FileUtils;
import video.jd.Jni.TrackUtils;
...
public class JVEditor {
	 ...
	 * 处理单个视频
	 *
	 * @param jvVideo      需要处理的视频
	 * @param outputOption 输出选项配置
	 */
	public static void exec(JVVideo jvVideo, OutputOption outputOption, OnEditorListener onEditorListener) {
		boolean isFilter = false;
		ArrayList<JVDraw> jvDraws = jvVideo.getEpDraws();
		//开始处理
		CmdList cmd = new CmdList();
		cmd.append("ffmpeg");
		cmd.append("-y");
		if (jvVideo.getVideoClip()) {
			cmd.append("-ss").append(jvVideo.getClipStart()).append("-t").append(jvVideo.getClipDuration()).append("-accurate_seek");
		}
		cmd.append("-i").append(jvVideo.getVideoPath());
		//添加图片或者动图
		if (jvDraws.size() > 0) {
			for (int i = 0; i < jvDraws.size(); i++) {
				if (jvDraws.get(i).isAnimation()) {
					cmd.append("-ignore_loop");
					cmd.append(0);
				}
				cmd.append("-i").append(jvDraws.get(i).getPicPath());
			}
			cmd.append("-filter_complex");
			StringBuilder filter_complex = new StringBuilder();
			filter_complex.append("[0:v]").append(jvVideo.getFilters() != null ? jvVideo.getFilters() + "," : "")
			...
			for (int i = 0; i < jvDraws.size(); i++) {
				filter_complex.append("[").append(i + 1).append(":0]").append(jvDraws.get(i).getPicFilter()).append("scale=").append(jvDraws.get(i).getPicWidth()).append(":")
						.append(jvDraws.get(i).getPicHeight()).append("[outv").append(i + 1).append("];");
			}
			for (int i = 0; i < jvDraws.size(); i++) {
				if (i == 0) {
				...
				filter_complex.append("overlay=").append(jvDraws.get(i).getPicX()).append(":").append(jvDraws.get(i).getPicY())
						.append(jvDraws.get(i).getTime());
				if (jvDraws.get(i).isAnimation()) {
			...
			StringBuilder filter_complex = new StringBuilder();
			if (jvVideo.getFilters() != null) {
				cmd.append("-filter_complex");
				filter_complex.append(jvVideo.getFilters());
				isFilter = true;
			}
			//设置输出分辨率
		...
		long duration = VideoUitls.getDuration(jvVideo.getVideoPath());
		if (jvVideo.getVideoClip()) {
			long clipTime = (long) ((jvVideo.getClipDuration() - jvVideo.getClipStart()) * 1000000);
	 ...
	 * 合并多个视频
	 *
	 * @param jvVideos     需要合并的视频集合
	 * @param outputOption 输出选项配置
	 */
	public static void merge(List<JVVideo> jvVideos, OutputOption outputOption, OnEditorListener onEditorListener) {
		//检测是否有无音轨视频
		boolean isNoAudioTrack = false;
		for (JVVideo jvVideo : jvVideos) {
			MediaExtractor mediaExtractor = new MediaExtractor();
			try {
				mediaExtractor.setDataSource(jvVideo.getVideoPath());
			} catch (IOException e) {
				e.printStackTrace();
				return;
			}
		...
		if (jvVideos.size() > 1) {
			...
			for (JVVideo e : jvVideos) {
				ArrayList<JVDraw> jvDraws = e.getEpDraws();
				if (jvDraws.size() > 0) {
					for (JVDraw jv : jvDraws) {
						if (jv.isAnimation()) cmd.append("-ignore_loop").append(0);
						cmd.append("-i").append(jv.getPicPath());
					}
				}
			}
			//添加滤镜标识
			cmd.append("-filter_complex");
			StringBuilder filter_complex = new StringBuilder();
			for (int i = 0; i < jvVideos.size(); i++) {
				StringBuilder filter = jvVideos.get(i).getFilters() == null ? new StringBuilder("") : jvVideos.get(i).getFilters().append(",");
			...
			//添加标记和处理宽高
			int drawNum = jvVideos.size();//图标计数器
			for (int i = 0; i < jvVideos.size(); i++) {
				for (int j = 0; j < jvVideos.get(i).getEpDraws().size(); j++) {
					filter_complex.append("[").append(drawNum++).append(":0]").append(jvVideos.get(i).getEpDraws().get(j).getPicFilter()).append("scale=")
							.append(jvVideos.get(i).getEpDraws().get(j).getPicWidth()).append(":").append(jvVideos.get(i).getEpDraws().get(j)
			...
			for (int i = 0; i < jvVideos.size(); i++) {
				for (int j = 0; j < jvVideos.get(i).getEpDraws().size(); j++) {
					filter_complex.append("[outv").append(i).append("][p").append(i).append("a").append(j).append("]overlay=")
							.append(jvVideos.get(i).getEpDraws().get(j).getPicX()).append(":")
							.append(jvVideos.get(i).getEpDraws().get(j).getPicY())
							.append(jvVideos.get(i).getEpDraws().get(j).getTime());
					if (jvVideos.get(i).getEpDraws().get(j).isAnimation()) {
						filter_complex.append(":shortest=1");
					}
			...
			filter_complex.append("concat=n=").append(jvVideos.size()).append(":v=1:a=0[outv]");
			//是否添加音轨
			if (!isNoAudioTrack) {
				...
				filter_complex.append("concat=n=").append(jvVideos.size()).append(":v=0:a=1[outa]");
			}
			if (!filter_complex.toString().equals("")) {
				cmd.append(filter_complex.toString());
			}
			...
			long duration = 0;
			for (JVVideo jv : jvVideos) {
				long d = VideoUitls.getDuration(jv.getVideoPath());
				if (jv.getVideoClip()) {
					long clipTime = (long) ((jv.getClipDuration() - jv.getClipStart()) * 1000000);
	 ...
	 * @param jvVideos         需要合并的视频的集合
	 * @param outputOption     输出选项
	 * @param onEditorListener 回调监听
	 */
	public static void mergeByLc(Context context, List<JVVideo> jvVideos, OutputOption outputOption, final OnEditorListener onEditorListener) {
